...
// social login
router.post('/social-login-api', async (req, res) => {
    ...
    if (duplicate_email_check) {
        var token1 = jwt.sign({ email: duplicate_email_check.email },'worldisfullofdevelopers', {
                expiresIn: '24h' // expires in 24 hours
            });
        return res.json({
            success: true,
            user: duplicate_email_check,
            email: duplicate_email_check.email,
            token: token1,
            code: 200,
        ...
        let token = jwt.sign({ email: req.body.email },'worldisfullofdevelopers', {
                expiresIn: '24h' // expires in 24 hours
            });
...
router.post("/assign-challan-all-api", VerifyToken, async(req, res) => {
    ...
    // console.log('assign_case',assign_case)
    ...
    //console.log('all_case_id',all_case_id);
...
// case details
router.post("/case-details-api", VerifyToken, async (req, res) => {
...
router.post("/conversation-api", profile1.single('conversationFile'), async (req, res) => {
    console.log((fileName === '') ? 'null' : `/upload_cases_file/${fileName}`);
...
router.post("/case-invitation-api", VerifyToken, async (req, res) => {
    const user_id = req.body.user_id;
    const case_id = req.body.case_id;
    ...
    await Notifications.create({
        name: name,
        remarks: `${invitation_id=='1'? accept : reject }`,
        status: 1,
        sender_id: user_id,
        receive_id: 1,
        img: image,
    ...
    res.json({
        code: 200,
        success: true,
        msg: invitation_id=='1'? accept : reject
    });
...
router.post("/forgot-password-api", async (req, res) => {
        ...
        var url = "http://legalkart.com/activate-account/" + activation_key;
        var duplicate_email_check = await User.findOne({
            where: {
                email: req.body.email
            }
        ...
        <!-- /body -->
            ...
            var request = await mailjet
                ...
                res.json({
                    code: 200,
                    success: true,
                    message: " Please cheack your email and reset your password"
                });
        } else {
            res.json({
                code: 500,
